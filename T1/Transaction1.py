...
class SupplyChain:
    ...
    def add_transaction(self,serial_no,engine_no,chassis_no,model,v_category,maf_batch,maf_date):
...
def form_supply():
        ...
        maf_batch = request.form["form_mf_batch"]
        maf_date = request.form["form_mf_date"]
      
        params = {"Serial No" : serial_no, "Engine No" : engine_no, "Chassis No" : chassis_no , "Model Name" : model, 
        "Vehicle Category" : v_category, "Manufacturing Batch" : maf_batch, "Manufacturing Date" : maf_date}
        json = params
        transaction_keys = ['Serial No', 'Engine No', 'Chassis No','Model Name','Vehicle Category','Manufacturing Batch','Manufacturing Date']
        if not all(key in json for key in transaction_keys):
            return 'Some elements of the transaction are missing', 400
