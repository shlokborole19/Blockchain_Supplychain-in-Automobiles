...
class SupplyChain:
    ...
    def add_transaction(self,serial_no,engine_no,chassis_no,model,v_category,colour,cust_name,sell_date,payment_amount,payment_ID,reg_no):
...
def form_supply():
        ...
        cust_name = request.form["form_cust_name"]
        sell_date = request.form["form_sell_date"]
        payment_amount = request.form["form_pay_amt"]
        payment_ID = request.form["form_pay_id"]
        reg_no = request.form["form_reg_no"]
      
        params = {"Serial No" : serial_no, "Engine No" : engine_no, "Chassis No" : chassis_no , "Model Name" : model, 
        "Vehicle Category" : v_category, "Colour" : colour, "Customer Name" : cust_name,"Sell Date" : sell_date,"Payment Amount" : payment_amount,"Payment ID" : payment_ID,"Vehicle Registration No" : reg_no}
        json = params
        transaction_keys = ['Serial No', 'Engine No', 'Chassis No','Model Name','Vehicle Category','Colour','Customer Name','Sell Date','Payment Amount','Payment ID','Vehicle Registration No']
        if not all(key in json for key in transaction_keys):
            return 'Some elements of the transaction are missing', 400
        index = supplychain.add_transaction(json['Serial No'], json['Engine No'], json['Chassis No'],json['Model Name'], json['Vehicle Category'], json['Colour'],json['Customer Name'],json['Sell Date'],json['Payment Amount'],json['Payment ID'],json['Vehicle Registration No'])
        response = {'message': f'This transaction will be added to Block {index}'}
